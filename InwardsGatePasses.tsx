...
import { Plus, Save, Trash2, Calculator, Search, ArrowDownLeft } from 'lucide-react';
...
export function InwardsGatePasses() {
  const [gpNo, setGpNo] = useState('20');
  ...
  const updateItem = (id: number, field: keyof GatePassItem, value: string | number) => {
    ...
    <div className="h-full flex flex-col p-1">
      ...
      <div className="bg-bg-header text-white px-3 py-1.5 flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <ArrowDownLeft className="h-4 w-4 text-accent-orange" />
          <span className="font-semibold text-sm">Inwards Gate Passes</span>
        </div>
      ...
      <div className="flex-1 flex flex-col gap-1 overflow-auto">
        ...
        <CustomCard className="p-2">
          <div className="grid grid-cols-4 gap-3">
            <div className="space-y-0.5">
              <Label className="text-text-secondary text-xs">GP No</Label>
              <Input
                value={gpNo}
        ...
        <CustomCard className="flex-1 flex flex-col p-2 min-h-0">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-white">Gate Pass Items</span>
            <Button
              onClick={addItem}
              size="sm"
